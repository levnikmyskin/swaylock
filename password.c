...
static void schedule_input_idle(struct swaylock_state* state) {
    ...
    state->input_idle_timer = loop_add_timer(state->eventloop, 1500, set_input_idle, state);
}
...
void schedule_auth_idle(struct swaylock_state* state) {
    ...
    state->auth_idle_timer = loop_add_timer(state->eventloop, 3000, set_auth_idle, state);
}
...
static void schedule_password_clear(struct swaylock_state* state) {
    ...
    state->clear_password_timer = loop_add_timer(state->eventloop, 10000, clear_password, state);
}
...
static void update_highlight(struct swaylock_state* state) {
    ...
    state->highlight_start = (state->highlight_start + (rand() % 1024) + 512) % 2048;
}

void swaylock_handle_key(struct swaylock_state* state, xkb_keysym_t keysym, uint32_t codepoint) {

    switch (keysym) {
        case XKB_KEY_KP_Enter: /* fallthrough */
        case XKB_KEY_Return: submit_password(state); break;
        case XKB_KEY_Delete:
