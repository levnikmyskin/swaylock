...
static void create_surface(struct swaylock_surface* surface) {
    ...
    surface->ext_session_lock_surface_v1 = ext_session_lock_v1_get_lock_surface(state->ext_session_lock_v1, surface->surface, surface->output);
    ext_session_lock_surface_v1_add_listener(surface->ext_session_lock_surface_v1, &ext_session_lock_surface_v1_listener, surface);
...
static void ext_session_lock_surface_v1_handle_configure(void* data, struct ext_session_lock_surface_v1* lock_surface, uint32_t serial, uint32_t width, uint32_t height) {
...
static void handle_wl_output_geometry(void* data, struct wl_output* wl_output, int32_t x, int32_t y, int32_t width_mm, int32_t height_mm, int32_t subpixel, const char* make,
...
static void handle_wl_output_name(void* data, struct wl_output* output, const char* name) {
    struct swaylock_surface* surface = data;
    surface->output_name             = strdup(name);
}

static void handle_wl_output_description(void* data, struct wl_output* output, const char* description) {
    // Who cares
}
...
static int parse_options(int argc, char** argv, struct swaylock_state* state, enum line_mode* line_mode, char** config_path) {
    ...
    static struct option long_options[] = {{"config", required_argument, NULL, 'C'},
    ...
    while (1) {
        int opt_idx = 0;
        c           = getopt_long(argc, argv, "c:deFfhi:kKLlnrs:tuvC:R:", long_options, &opt_idx);
        if (c == -1) {
            break;
        }
        switch (c) {
            ...
            case 'c':
                ...
                break;
            case 'd': swaylock_log_init(LOG_DEBUG); break;
            case 'e':
                if (state) {
...
void log_init(int argc, char** argv) {
    static struct option long_options[] = {{"debug", no_argument, NULL, 'd'}, {0, 0, 0, 0}};
    int                  c;
    optind = 1;
    while (1) {
        int opt_idx = 0;
        ...
        switch (c) {
            case 'd': swaylock_log_init(LOG_DEBUG); return;
        }
    }
