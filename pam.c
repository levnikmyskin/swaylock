...
static int handle_conversation(int num_msg, const struct pam_message** msg, struct pam_response** resp, void* data) {
...
static const char* get_pam_auth_error(int pam_status) {
    switch (pam_status) {
        case PAM_AUTH_ERR: return "invalid credentials";
        ...
        case PAM_MAXTRIES: return "maximum number of authentication tries exceeded";
        default:;
            static char msg[64];
...
void run_pw_backend_child(void) {
    ...
    while (1) {
        ...
        if (!success) {
            swaylock_log(LOG_ERROR, "pam_authenticate failed: %s", get_pam_auth_error(pam_status));
        }
