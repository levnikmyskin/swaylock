...
void loop_poll(struct loop* loop) {
    ...
    if (!wl_list_empty(&loop->timers)) {
        ...
        wl_list_for_each_safe(timer, tmp_timer, &loop->timers, link) {
            ...
            bool expired = timer->expiry.tv_sec < now.tv_sec || (timer->expiry.tv_sec == now.tv_sec && timer->expiry.tv_nsec < now.tv_nsec);
            if (expired) {
...
void loop_add_fd(struct loop* loop, int fd, short mask, void (*callback)(int fd, short mask, void* data), void* data) {
...
struct loop_timer* loop_add_timer(struct loop* loop, int ms, void (*callback)(void* data), void* data) {
...
bool loop_remove_fd(struct loop* loop, int fd) {
    ...
    wl_list_for_each_safe(event, tmp_event, &loop->fd_events, link) {
        if (loop->fds[fd_index].fd == fd) {
            ...
            loop->fd_length--;
            memmove(&loop->fds[fd_index], &loop->fds[fd_index + 1], sizeof(struct pollfd) * (loop->fd_length - fd_index));
            return true;
        }
        ++fd_index;
    }
